...
const displayedValue = computed(() => {
  if (isFocused.value) return localInput.value
...
onMounted(() => {
  if (inputRef.value) inputRef.value.value = displayedValue.value
})
...
function extractLastValidNumber(str: string) {
  ...
  if (!props.canBeNegative && last.startsWith('-')) last = last.slice(1)
  last = last.replace(',', '.')
